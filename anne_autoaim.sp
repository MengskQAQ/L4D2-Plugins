...
//int iPlayerButtons[MAXPLAYERS + 1];
...
//			if (c_bDebug_Enabled) PrintToChatAll("\x01[%.2f] \x05%N \x01Cap Smoker: \x04%N (%d)", GetGameTime(), client, aCapSmoker, aCapSmoker);
			...
			if (aimdist < 80.0) buttons |= IN_ATTACK2;
/*			else {
				buttons &= ~IN_ATTACK2;
				buttons |= IN_DUCK;
			}
...
//					CreateDataTimer(0.05, Timer_LagShoot, data);
...
//https://forums.alliedmods.net/showthread.php?t=297789&page=3
...
{ 
	...
	buttons |= IN_ATTACK;
//	Action ret = Plugin_Continue;
//	ret = onAttackRunCmd(client, buttons);
//	return ret;
}

/*stock Action onAttackRunCmd(int client, int &buttons)
{
	buttons |= IN_ATTACK;
	return Plugin_Changed;
}*/
